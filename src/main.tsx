...
async function run(flush_ani_data: boolean = true): Promise<void> {
    ...
    if (flush_ani_data) {
        if (
            Math.abs(
                new Date().getTime() -
                    store.collection_dict_update_time.getTime()
            ) >
            7 * 24 * 60 * 60 * 1000
        )
        ...
        await flush_user_collection_cache(
            username,
            Subject_type.动画,
            Collections_type.想看
        )
