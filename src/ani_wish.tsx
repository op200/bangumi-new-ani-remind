...
async function render(
    ...
    while (true) {
        ...
        try {
            ...
            collections_filted = store.collection_dict[COLLECTION_KEY].filter(
                (value, index, array) => {
                    ...
                    return collections_filted_function(
                        new Date(value.subject.date).getTime() /
                            (24 * 60 * 60 * 1000),
            ...
            >
                {collections_filted.length ? (
                    collections_filted
                        .sort(
                            (d1, d2) =>
                                new Date(d1.subject.date).getTime() -
                                new Date(d2.subject.date).getTime()
                        )
                        .map(data => (
                            <a
                                className="thumbTip"
                                href={`/subject/${data.subject_id}`}
